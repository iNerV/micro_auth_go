...
MIGRATE := docker run -v $(shell pwd)/internal/database/migrations:/migrations --network host migrate/migrate -path=/migrations/ -database "$(APP_DSN)?sslmode=disable"


LINT := docker run --rm -v $(shell pwd):/app -w /app golangci/golangci-lint:v1.23.6 golangci-lint run -v
...
test: ## run unit tests
	...
	@$(foreach pkg,$(PACKAGES), \
		go test -p=1 -cover -v -coverpkg ./... -covermode=count -coverprofile=coverage.out ${pkg}; \
...
.PHONY: lint
lint: ## run golang-ci-lint on all Go package
...
migrate-new: ## create a new database migration
	...
	$(MIGRATE) create -seq -ext sql -dir /internal/database/migrations/ $${name// /_}
